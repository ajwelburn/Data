...
import io
import math # Added for the exponential function in W'bal calculation
...
def parse_fit_file(uploaded_file):
    ...
    # Ensure data is second-by-second by resampling and forward-filling
...
# NEW: W' Balance Calculation Function
def calculate_w_prime_balance(power_series, cp, w_prime, A, B):
    ...
    for p in power_series:
        ...
        # Ensure W'bal doesn't exceed its maximum or go below zero
...
# NEW: W' Balance Plotting Function
...
if uploaded_files:
    ...
    if all_bouts_list:
        ...
        st.download_button(
            label="📥 Download Bout Analysis as Excel File",
            data=excel_data,
