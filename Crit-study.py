...
def calculate_matches_and_zones(w_bal_series, w_prime, time_series):
    ...
    for i, val in enumerate(w_bal_series):
        if val < match_threshold and not below_threshold:
            match_count += 1
            match_times.append(time_series.iloc[i])
    ...
    # Handle potentially empty series
...
def plot_w_prime_balance(time_series, w_bal_series, w_prime):
    ...
    ax.set_ylim(0, 105); ax.set_xlim(0, time_series.max() if not time_series.empty else 1)
    ax.grid(alpha=0.3)
    return fig
...
def generate_excel_output(bouts_df, cp, w_prime, all_files_data):
    ...
    with ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # --- FIX IS HERE: Robustly consolidate individual zone data ---
        ...
        for file_data in all_files_data:
            if not file_data['zone_data'].empty:
                ...
                # Turn the index (e.g., '0-10%') into a column named 'Zone'
        ...
        summary_metrics = {
            ...
            'Value': [
                len(bouts_df) if not bouts_df.empty else 0,
                bouts_df['magnitude'].mean() if not bouts_df.empty else 0,
                bouts_df['duration'].mean() if not bouts_df.empty else 0,
                sum(f['match_count'] for f in all_files_data) if all_files_data else 0
            ]
        }
