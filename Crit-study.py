...
def create_summary_plots(bouts_df, cp, w_prime, title_prefix=""):
    ...
    # This loop contained the syntax error. It is now fixed.
    for depletion in range(10, 60, 10):
        y_values = []
        for t in range(1, 71):
            w_prime_depleted = w_prime * (depletion / 100)
            power_for_depletion = (w_prime_depleted / t) + cp
            magnitude = (power_for_depletion / cp) * 100
...
def generate_excel_output(all_files_data, cp, w_prime):
    ...
    with ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if all_zone_rows:
            consolidated_zones_df = pd.DataFrame(all_zone_rows)
